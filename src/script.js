  ...
  particlesJS("particles-js", {
    particles: {
      ...
      opacity: {
        value: 0.15,
        random: true,
        anim: { enable: true, speed: 0.2, opacity_min: 0.03, sync: false },
      },
...
document.getElementById("githubIssueLink").href =
  `https://github.com/${GITHUB_REPO_USER}/${GITHUB_REPO_NAME}/issues`;

const endpoints = [
  // Example endpoints from er-api.biz.id
  {
    title: "OpenRouter AI (Mistral)",
    service: "/ai/mistralai/mistral-7b-instruct:free",
    q: "t=Who are you?",
    method: "GET",
  ...
  },
  {
    ...
    service: "/get/run",
    q: "c=console.log('Hello World')&lang=javascript",
    method: "GET",
    categoryName: "Code Runners",
  },
  {
    title: "Tiktok Downloader",
    service: "/dl/tt",
    q: "u=https://vt.tiktok.com/ZSML4JTrb/",
    method: "GET",
    categoryName: "Downloaders",
  },
  ...
  {
    title: "Wikipedia.org",
    service: "https://www.wikipedia.org",
    q: "",
    method: "GET",
    categoryName: "Public Websites",
  },
  ...
  {
    title: "Microsoft.com",
    service: "https://www.microsoft.com",
    q: "",
    method: "GET",
    categoryName: "Public Websites",
  },
  ...
  {
    title: "Invalid URL (404 Test)",
    service: "https://www.example.com/non-existent-page-123",
    q: "",
    method: "GET",
    categoryName: "Test Endpoints",
  },
  {
    title: "Invalid Domain (Network Error Test)",
    service: "http://this-domain-does-not-exist-123456.com",
    q: "",
    method: "GET",
    categoryName: "Test Endpoints",
  },
...
function renderVisibleEndpoints() {
  ...
  const uniqueCategories = [
    ...new Set(
      allProcessedEndpointData.map((item) => item.endpoint.categoryName),
    ),
  ];
  ...
  uniqueCategories.forEach((categoryName) => {
    ...
    allProcessedEndpointData.forEach((item) => {
      if (
        item.endpoint.categoryName === categoryName &&
        displayedCount < numCurrentlyVisibleGlobalEndpoints
      ) {
  ...
  if (numCurrentlyVisibleGlobalEndpoints >= allProcessedEndpointData.length) {
    ...
    loadMoreButtonGlobal.innerHTML =
...
function handleLoadMoreGlobal() {
  ...
  setTimeout(() => {
    const nextBatchEnd = Math.min(
      numCurrentlyVisibleGlobalEndpoints + ITEMS_PER_PAGE,
      allProcessedEndpointData.length,
    );
...
function openAiModalForLanguageSelection(type) {
  ...
  if (type === "error_help") {
    aiModalTitle.innerHTML =
      '<i class="fas fa-language mr-2" style="color: var(--accent-blue);"></i> Select Error Help Language';
  } else if (type === "explanation") {
    aiModalTitle.innerHTML =
      '<i class="fas fa-language mr-2" style="color: var(--accent-blue);"></i> Select Endpoint Explanation Language';
  }
...
copyAiResponseButton.addEventListener("click", () => {
  const textToCopy =
    aiResponseContent.innerText || aiResponseContent.textContent;
  copyBuffer.value = textToCopy;
  copyBuffer.select();
  try {
    document.execCommand("copy");
    copyAiResponseButton.innerHTML =
...
async function callGeminiApi(prompt, language) {
  ...
  if (currentAiRequestData.type === "error_help") {
    aiModalTitle.innerHTML =
      '<i class="fas fa-lightbulb mr-2" style="color: var(--accent-yellow);"></i> AI Error Help';
  } else if (currentAiRequestData.type === "explanation") {
    aiModalTitle.innerHTML =
      '<i class="fas fa-info-circle mr-2" style="color: var(--accent-blue);"></i> Endpoint Explanation';
  }
  ...
  try {
    ...
    if (
      ...
      aiResponseContent.textContent =
        "No suggestions received from AI or response format is unexpected.";
...
async function checkEndpointAndCreateRow(endpoint) {
  ...
  const statusBadgeContainer = rowElement.querySelector(
  ...
  explainButton.className =
    "action-button action-button-explain explain-endpoint-button";
  ...
  try {
    ...
    if (!response.ok) {
      ...
      aiHelpButton.className =
        "action-button action-button-ai ai-error-help-button";
    ...
    if (error.name === "TimeoutError") {
      errorMsg = "Timeout";
      detailedErrorCause = "Request to server exceeded 20 seconds timeout.";
    } else if (
      error instanceof TypeError &&
      error.message.toLowerCase().includes("failed to fetch")
    ) {
      errorMsg = "Network/CORS";
      detailedErrorCause =
        "Failed to fetch data. Possible causes: CORS issue, server unreachable, DNS, or SSL. Check browser console (Network Tab).";
    } else {
      detailedErrorCause = error.message || "Unknown error";
    }
...
async function loadAllEndpoints(isAutoRefresh = false) {
  ...
  updateOverallStatusDisplay();
  lastUpdatedElement.textContent = isAutoRefresh
    ? `Auto-refreshing... (${new Date().toLocaleTimeString("en-US")})`
    : "Loading...";
  ...
  if (!isAutoRefresh) {
    numCurrentlyVisibleGlobalEndpoints = Math.min(
      ITEMS_PER_PAGE,
      allProcessedEndpointData.length,
    );
  }
  ...
  lastUpdatedElement.textContent = new Date().toLocaleString("en-US", {
    dateStyle: "long",
    timeStyle: "medium",
  });
  updateOverallStatusDisplay();
...
function startAutoRefresh() {
  ...
  autoRefreshIntervalId = setInterval(
    () => loadAllEndpoints(true),
    AUTO_REFRESH_INTERVAL,
  );
  console.log(
    `Auto-refresh enabled. Interval: ${AUTO_REFRESH_INTERVAL / 1000} seconds.`,
  );
}
...
function updateOverallStatusDisplay() {
  ...
  if (
    ...
    checkedEndpointsCounter < totalEndpointsCount
  ) {
    ...
    operationalEndpointsCounter === totalEndpointsCount &&
    totalEndpointsCount > 0
  ) {
